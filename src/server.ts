...
server.registerTool(
  "function-create",
  {
    ...
    description: "Create a new function in Spica",
    inputSchema: {
      name: z.string(),
      code: z.string(),
      triggers: z.optional(z.array(z.any())),
      dependencies: z.optional(z.record(z.string())),
      environment: z.optional(z.record(z.string())),
    },
  },
  async ({
    name,
    ...
    try {
      ...
      return {
        content: [
          {
            type: "text",
            text: `❌ Failed to create function:\n${err.message}`,
          },
        ],
      };
    }
...
server.registerTool(
  "function-update",
  {
    ...
    description: "Update an existing function in Spica",
    inputSchema: {
      functionId: z.string(),
  ...
  async ({ functionId, ...updateData }) => {
    try {
      // First get the current function data
      const currentResponse = await makeSpicaRequest(
        "GET",
        `/function/${functionId}`
      );
      ...
      // Merge with update data
      const mergedData = { ...currentData, ...updateData, _id: functionId };
      ...
      return {
        content: [
          {
            type: "text",
            text: `❌ Failed to update function:\n${err.message}`,
          },
        ],
      };
    }
...
server.registerTool(
  "function-delete",
  {
    ...
    description: "Delete a function from Spica",
    inputSchema: {
      functionId: z.string(),
    },
  },
